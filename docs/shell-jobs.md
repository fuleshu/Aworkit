...
Enable **Start shell job**, **Read job output**, **Send job input**, **Stop shell job**,
and **List shell jobs** in Settings and bind them to the Agent. Add **Keep shell job
running** when the Agent may deliberately leave a service available. Start a new
Chat: existing Chats keep their frozen tool selections and permissions.
...
Windows shells launch without a console window. GUI apps can still show their own
windows. Windows Job Objects track descendants even after the root shell exits;
...
tests, and `desktop/scripts/native-shell-jobs-smoke.mjs` exercise real Windows
