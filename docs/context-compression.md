...
3. Extend the canonical native plugin manifest with `tool.context` (`context`), operations read/search/stats. Retrieval takes an opaque reference, optional JSON pointer or UTF-8 byte offset, and bounded page size; search returns ranked exact spans. No model-supplied Chat/node/owner selector. Results of retrieval are never compressed again. All arguments and grants pass through the existing broker.
...
In **Settings → Providers & models**, open a model's **Tool result compression** panel. The settings are frozen when a new Chat starts. New Chats default to lossless mode; existing frozen Chats without a compression policy do not run optional compression. The mandatory tool-output cap still applies.
...
The pipeline runs on a **newly settled tool result**, before the existing model output cap and before the next context-pressure check. It does not run every time old history is serialized. It admits a compression representation only when both counted tokens and bytes improve by the minimum savings, including format instructions and reference metadata. Adaptive selection competes with the lossless representation; an extraction failing the final token gate falls back to lossless. Inputs larger than 512 KiB pass through to the existing tool limits. Other work bounds include 8,192 rows/64 columns per reversible table, 2,048 adaptive rows or spans, nesting depth 16, and a 100 ms syntax parse limit. An unsuccessful transformation leaves the result for the mandatory output-bound fallback below.

### Mandatory output-bound fallback

If a newly settled result still exceeds the configured output limit, Aworkit returns valid JSON containing `aworkitOutput` metadata and an explicitly partial `preview`. It reports original bytes, the limit and a narrower-query instruction. Small factual fields and fitting metadata are retained; string suffixes carry a truncation marker. Arrays may omit items, so preview positions and lengths must not be interpreted as original indices or complete counts. Opaque media and annotated blocks are retained whole where they fit or omitted whole; their contents are never sliced into invalid media. The canonical broker outcome remains unchanged.

For an eligible result with compression enabled and Context retrieval selected, this fallback commits the original and preview through the existing archive before publishing an `aworkitContext` reference. It is recorded as strategy `output-limit-preview` with `lossy: true`, including when optional compression is configured as lossless: it replaces the old mandatory byte cut, not the lossless compression guarantee. With compression off, a missing policy, an excluded capability, or unavailable retrieval, the final cap still returns a valid partial preview but does not add retrieval authority or claim a retrievable reference. Existing earlier messages are not reprojected.
...
`tool.context` has provider name `context` and a configurable `maximumBytes` of 256–65,536 (default 16,384), further bounded by the Agent output limit:
