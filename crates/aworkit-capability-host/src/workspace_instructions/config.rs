...
impl Configuration {
    ...
    pub fn resolve(mut self, home: Option<&Path>) -> Result<Self, String> {
        ...
        self.aworkit_home = if value.is_empty() {
            home.ok_or("user home unavailable; configure Aworkit home")?
                .join(".aworkit")
    ...
    pub fn validate(&self) -> Result<(), String> {
        ...
        if self.max_bytes > 65536 || self.max_source_bytes == 0 || self.max_source_bytes > 1048576 {
            return Err(
                "instruction budget must be 0..65536 and source limit 1..1048576 bytes".into(),
            );
        }
        for list in [
            ...
            if list.len() > 64 || list.iter().any(|s| s.len() > 255) {
                return Err(
                    "instruction candidate lists exceed 64 names or 255 bytes per name".into(),
                );
            }
        }
        Ok(())
    ...
    pub fn diagnostics(&self) -> Vec<String> {
        self.project_root_markers
            .iter()
            .chain(&self.instruction_file_candidates)
            .chain(&self.local_instruction_file_candidates)
            .filter(|name| !valid_name(name))
            .map(|name| format!("Ignored unusable instruction candidate: {name:?}"))
            .collect()
    }
