...
use super::{
    Action, Change, Configuration, FileObservation, InstructionFiles, RenderItem, check_cancelled,
    digest, render,
};
...
fn effective<'a>(events: impl Iterator<Item = &'a Event>, durable: bool) -> Effective<'a> {
    ...
    for event in events {
        if event.baseline && (!durable || event.reset) {
            state.clear();
        }
...
pub fn prepare(request: Preparation<'_>) -> Result<Plan, String> {
    ...
    let Some(files) = files else {
        plan.diagnostics
            .push("Workspace instructions unavailable: no authorized filesystem provider".into());
        return Ok(plan);
    };
    let project = workspace
        .zip(cwd)
        .filter(|(root, cwd)| cwd.starts_with(root));
    ...
    if let Some((boundary, cwd)) = project {
        'ancestors: for directory in directories(boundary, cwd).into_iter().rev() {
            for marker in config
                ...
                .iter()
                .filter(|s| super::config::valid_name(s))
            {
                check_cancelled(cancellation)?;
                match files.exists(&directory.join(marker), cancellation) {
                    ...
                    Err(error) => plan
                        .diagnostics
                        .push(format!("Root marker unavailable: {error}")),
                }
            }
        }
    }
    let identity = digest(
        &serde_json::to_string(&(config, workspace, cwd, &root)).map_err(|e| e.to_string())?,
    );
    let owned: Vec<_> = history
        .iter()
        .filter(|e| e.owner == *owner && e.producer == "aworkit.workspace_instructions.v1")
        .collect();
    ...
    let visible: Vec<_> = owned
        .iter()
        .copied()
        .filter(|e| selection.event_ids.contains(&e.id))
        .collect();
    plan.retain = visible.iter().map(|e| e.id.clone()).collect();
    let visible_baseline = visible.iter().rev().find(|e| e.baseline).copied();
    let mut baseline = !compatible || visible_baseline.is_none_or(|e| e.identity != identity);
    let known = effective(owned.iter().copied(), true);
    let current = effective(visible.iter().copied(), false);
    let lost = known
        .iter()
        .any(|(scope, (_, change))| current.get(scope).is_none_or(|(_, c)| c != change));
        ...
        if compatible {
            for event in &owned {
                if event.identity == identity {
                    scopes.extend(event.observed_directories.iter().cloned());
                }
            }
        }
        for touch in settled_touches {
            let path = if touch.is_absolute() {
                touch.clone()
            } else {
                cwd.join(touch)
            };
    ...
    if scopes.len() > 8192 {
        return Err("workspace instruction directory budget exceeded".into());
    }
    // Omitted candidates retain their discovery scope and remain retryable.
    let pending = owned.last().is_some_and(|e| e.pending);
    if !baseline && !lost && !refresh && settled_touches.is_empty() && !pending {
        return Ok(plan);
    }
    let mut ordered: Vec<_> = scopes.iter().cloned().collect();
    ordered.sort_by_key(|s| (s.split('/').count(), s.clone()));
    let mut candidates = vec![(
        "user-global".to_owned(),
        "AGENTS.md".to_owned(),
        config.aworkit_home.join("AGENTS.md"),
        display(&config.aworkit_home.join("AGENTS.md")),
        false,
    )];
    ...
    for (dir, name, path, label, nested) in candidates {
        ...
        let prior = current
            .get(&scope)
            .filter(|(_, c)| c.action != Action::Remove);
        let durable = compatible
            .then(|| known.get(&scope))
            .flatten()
            .filter(|(_, c)| c.action != Action::Remove);
        ...
        let content = match observation {
            FileObservation::Present(text) => {
                available = true;
                if !seen_text.insert((dir, digest(text.trim()))) {
                    None
                } else {
                    Some((text.clone(), digest(&text)))
                }
            }
            ...
            FileObservation::Unavailable(error) => {
                plan.diagnostics.push(format!(
                    "Instructions temporarily unavailable: {label}: {error}"
                ));
                if let Some((event, change)) = durable.or(prior) {
                    if !baseline && prior.is_some() {
                        continue;
                    }
        ...
        let (action, content, fingerprint) = match content {
            Some((text, fingerprint)) => {
                if !baseline
                    && prior.is_some_and(|(_, c)| c.digest.as_deref() == Some(&fingerprint))
                {
                    continue;
                }
                (
            ...
            None if prior.is_some() || (baseline && durable.is_some()) => {
                (Action::Remove, String::new(), None)
            }
            None => continue,
        };
    ...
    check_cancelled(cancellation)?;
    if !available && items.is_empty() {
        return Ok(plan);
    }
    if baseline && items.is_empty() && previous_baseline.is_none() {
        baseline = false;
    }
    let rendered = if baseline || !items.is_empty() {
        render(
            &items,
            config.max_bytes,
            baseline,
            previous_baseline.is_some() && (!compatible || visible_baseline.is_some()),
        )
    } else {
        Default::default()
    };
    ...
    plan.event = Some(Event {
        id: event_id.into(),
        producer: "aworkit.workspace_instructions.v1".into(),
        owner: owner.clone(),
        identity,
        baseline,
        reset: !compatible,
        pending: items.len() > rendered.changes.len(),
        text: rendered.text,
