...
fn heading(item: &RenderItem, baseline: bool) -> String {
    ...
    if baseline && !item.nested && change.action != Action::Remove {
        return format!("Instructions from: {}\n\n", change.path);
    }
    match change.action {
        Action::Set => format!(
            ...
            change.path,
            change.scope.split('\0').next().unwrap_or(".")
        ),
...
fn marker(items: &[RenderItem], start: usize, included: Option<usize>, budget: usize) -> String {
    ...
    if parts.is_empty() {
        ...
        format!(
            "Workspace instruction budget {budget} bytes: {}",
            parts.join("; ")
        )
    }
}
...
fn build(
    ...
    for block in [&notice, intro] {
        if !block.is_empty() {
            ...
            result.text.push_str(&escape(block));
            blocks += 1;
        }
    }
...
pub fn render(items: &[RenderItem], budget: usize, baseline: bool, supersede: bool) -> Rendered {
    ...
    for style in [intro.as_str(), COMPACT] {
        ...
        while low <= high {
            ...
            if build(items, start, Some(bytes), budget, style, baseline, true)
                .text
                .len()
                <= budget
            {
