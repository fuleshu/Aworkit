...
#[test]
fn request_validation_bounds_image_metadata_without_an_image_budget() {
    ...
    // Forty 4 MiB images — 160 MiB of image context — are a valid request. A
    // request is never rejected for image count or total image bytes; the
    // bound lives where images are attached to a provider dispatch, which keeps
    // the newest images and refers to the rest.
    request
        .validate()
