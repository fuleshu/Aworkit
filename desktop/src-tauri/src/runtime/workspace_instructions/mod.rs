...
use aworkit_capability_host::{
    ModelToolContextV1, ModelToolRequestV1,
    workspace_instructions::{
        self as library, Configuration, Event, Owner, Preparation, ProjectInstructionFiles,
        Selection,
    },
};
...
impl BoundFileToolAuthorityV1 {
    ...
    pub(super) fn child_instruction_context(
        &self,
        invocation: &StableId,
    ) -> Result<AgentContextV1, String> {
        ...
        let steps: Vec<Step> = self
            ...
            .collect::<Result<_, _>>()
            .map_err(|e| e.to_string())?;
        let selected = parent.as_ref().and_then(|parent| {
            steps
                .iter()
                .rev()
        ...
        Ok(AgentContextV1 {
            ...
            tool_ids: self
                ...
                .iter()
                .filter(|b| {
                    SUBAGENT_CHILD_TOOL_IDS.contains(&b.capability_id.as_str())
                        && (b.is_callable() || selected.is_some())
                })
    ...
    pub(super) fn workspace_context(
        ...
        if !agent.tool_ids.iter().any(|id| id == ID) {
            return Ok(());
        }
        let binding = self
            .context
            ...
            .ok_or("missing frozen Workspace Instructions binding")?;
        let StoredFileToolLimitV1::WorkspaceInstructions { configuration } = &binding.limit else {
            return Err("invalid automatic context binding".into());
        };
        ...
        let owner = Owner {
            ...
            context: format!(
                "{}:{}",
                self.context.project_branch.as_deref().unwrap_or(""),
        ...
        let lock = {
            let mut locks = self
                ...
                .instruction_locks
                .lock()
                .map_err(|_| "instruction owner lock poisoned")?;
            locks.retain(|_, lock| lock.strong_count() > 0);
            let key = serde_json::to_string(&owner).map_err(|e| e.to_string())?;
        ...
        if let Some(step) = steps.iter().find(|s| {
            s.outer == *outer && s.after_exchanges == after_exchanges && s.revision == revision
        }) {
        ...
        for message in &request.context_messages {
            if let Some(id) = &message.instruction_event_id {
                if history.iter().any(|e| {
                    e.id == *id
                        && e.text == message.content
                        && message.role.as_deref().is_none_or(|r| r == "user")
                        && message.images.is_empty()
                }) {
                    positions.push(Position {
                        id: id.clone(),
                        after_exchanges: message.after_exchanges,
                        after_input_messages: message.after_input_messages,
                    });
                }
            }
        }
        let input = request.input["messages"]
            .as_array()
            .ok_or("Agent context requires input.messages")?;
        let system_count = input.iter().take_while(|m| m["role"] == "system").count();
        let after_conversation = self
            .context
            .review_messages
            .iter()
        ...
        for step in &steps {
            ...
            if positions.iter().any(|p| p.id == event.id) {
                continue;
            }
            if revision != "ordinary" && step.revision != revision {
                continue;
            }
            ...
            let position = if same_input {
                Position {
                    id: event.id.clone(),
                    after_exchanges: step.after_exchanges
                        + request.exchanges.len().saturating_sub(after_exchanges),
                ...
                Position {
                    id: event.id.clone(),
                    after_exchanges: 0,
                    after_input_messages: Some(
                        (system_count + step.after_conversation).min(input.len()),
                    ),
                }
            } else {
                Position {
                    id: event.id.clone(),
                    after_exchanges: request.exchanges.len().saturating_sub(after_exchanges),
                    after_input_messages: None,
                }
            };
            positions.push(position);
        ...
        let selection = Selection {
            ...
            event_ids: positions.iter().map(|p| p.id.clone()).collect(),
        };
        let workspace = self
            .context
            .approvals
            .project_key
            .as_ref()
            .map(|_| self.context.workspace.root.as_path());
        let validation = self
            .runtime
            .projects
        ...
        let event_id = digest_id(
            "record.workspace-instructions",
            &format!(
                "{}:{}:{outer}:{after_exchanges}:{revision}",
                owner.node, owner.context
            ),
        )
        .map_err(|e| e.to_string())?;
        let mut plan = library::prepare(Preparation {
            ...
            files: validation
                .is_ok()
                .then_some(&files as &dyn library::InstructionFiles),
            history: &history,
            selection: &selection,
            event_id: event_id.as_str(),
            settled_touches: &settled_touches,
            refresh: after_exchanges == 0,
            cancellation,
        })?;
        ...
        self.runtime
            .records
            .append(KIND, &event_id, value)
            .map_err(|e| e.to_string())?;
        self.run_events.publish_instruction_context(
            &agent.node_id,
            &serde_json::to_value(&step.event).map_err(|e| e.to_string())?,
            &step.diagnostics,
        )?;
    ...
    fn settled_instruction_touches(
        &self,
        outer: &StableId,
        through: usize,
        ...
        let calls: BTreeSet<String> = exchanges
            .iter()
            .filter(|e| e["outerInvocationId"] == outer.as_str())
            .filter(|e| e["turn"].as_u64().is_some_and(|t| (t as usize) == through))
            .filter_map(|e| e["exchange"]["assistantContent"].as_array())
            .flatten()
        ...
        Ok(identities
            ...
            .filter(|(id, _)| {
                outcomes
                    .iter()
                    .any(|o| o.invocation_id == *id && !o.is_error)
            })
            .filter_map(|(_, i)| i.call.arguments["path"].as_str().map(PathBuf::from))
            .collect())
    }
}
...
fn project(
    ...
    for message in &mut request.context_messages {
        if message.instruction_event_id.as_ref().is_some_and(|id| {
            !history
                .iter()
                .any(|e| e.id == *id && e.text == message.content)
        }) {
    ...
    for position in positions {
        ...
        let event = history
            .iter()
            .find(|e| e.id == position.id)
            .ok_or("instruction context references an uncommitted event")?;
        if event.text.is_empty() {
            continue;
        }
        request.context_messages.push(ModelToolContextV1 {
            after_exchanges: position.after_exchanges,
            after_input_messages: position.after_input_messages,
            instruction_event_id: Some(event.id.clone()),
