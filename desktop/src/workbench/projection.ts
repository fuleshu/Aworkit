...
export const coreEventSchema = z
  ...
  .strict();
export const coreReceiptSchema = z
  .object({
    commandId: stableId,
    accepted: z.boolean(),
...
export interface ProjectionReducer<T> {
  readonly initial: T;
  reduce(model: T, event: CoreEvent): T;
}
...
export class ProjectionGateway<T> {
  ...
  public dispatch(commandId: string): ProjectionState<T> {
    this.state = {
      ...this.state,
      pendingCommands: new Set([...this.state.pendingCommands, commandId]),
    };
    return this.state;
  }
  public receiveReceipt(input: unknown): ProjectionState<T> {
    const receipt = coreReceiptSchema.parse(input);
    const pending = new Set(this.state.pendingCommands);
    pending.delete(receipt.commandId);
    this.state = { ...this.state, pendingCommands: pending };
    return this.state;
  }
  public receiveEvent(input: unknown): ProjectionState<T> {
    ...
    this.state = {
      ...
      model: this.reducer.reduce(this.state.model, event),
    };
    return this.state;
  }
  public resynchronize(sequence: number, model: T): ProjectionState<T> {
    if (!Number.isSafeInteger(sequence) || sequence < this.state.sequence)
      throw new Error("projection snapshot is stale");
    this.state = { ...this.state, sequence, model, stale: false };
    return this.state;
  }
}
