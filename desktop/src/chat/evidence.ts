...
export interface EvidenceQuery {
  readonly filter?: EvidenceRecord["category"];
...
export function queryEvidence(
  ...
  const filtered =
    query.filter === undefined
      ? records
      : records.filter((record) => record.category === query.filter);
  return {
    total: filtered.length,
    items: filtered.slice(
      Math.max(0, query.offset),
      Math.max(0, query.offset) + Math.max(1, query.limit),
    ),
  };
}
...
export function inspectEvidence(record: EvidenceRecord): string {
  if (record.state !== "available")
    return `Evidence is ${record.state}. No additional value is inferred.`;
  return JSON.stringify(record.value, null, 2);
}

export function redactedEvidenceJson(record: EvidenceRecord): string {
